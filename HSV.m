function [] = HSV(files,altref,start,dur,stop,plotmode,gemplot)
%default behavior -> HSV('./datafolder',175,0,10,300,'both',false);
...
arguments
    ...
    gemplot (1,1) int16 = 0 %Should we run gemini plotting utilities? 
end

files=char(files);
plotmode=char(plotmode);

sized=10;    % label size

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if exist('gemplot','var')
    if gemplot
	fprintf('Running regular GEMINI plotting routine...\n');
	gemini3d.plot.plotall(files,'png');
	fprintf('Done/n/n')
    end
end
...
if strcmp(plotmode,'both')
    ...
    HSV(files,altref,start,dur,stop,'BOTH_Auto')
    fprintf('Running Standard \n');
    HSV(files,altref,start,dur,stop,'BOTH_Standard')
    return
end
...
for UTsec=start:dur:stop
    ...
    data = gemini3d.read.frame([files,filesep,names(:,:,it)]);
    ...
    parmp3V2=interp2(xg.x3(inds3),xg.x1(inds1),parmtmp,x3plot,x1plot);
    parmp3V2=reshape(parmp3V2,[lzp,lyp]);    %slice expects the first dim. to be "y"
    
    parmtmp=squeeze(parmV3(:,ix2,:));     %so north dist, east dist., alt.
    parmp3V3=interp2(xg.x3(inds3),xg.x1(inds1),parmtmp,x3plot,x1plot);
    parmp3V3=reshape(parmp3V3,[lzp,lyp]);    %slice expects the first dim. to be "y"
    ...
    koutu = parmp3V2(1:53,:);                               % E-W velocity
    koutv = parmp3V3(1:53,:);                               % N-S velocity
    ...
    figure(1);
    set(gcf,'PaperPosition',[0,0,20,6],'PaperUnits','inches')
    ...
    cat1=subplot(3,3,1);
    h=imagesc(mlat,zp,parmp3V3);
    set(h,'alphadata',~isnan(parmp3V3));
    set(gca,'FontSize',sized)
    axis xy
    clb=colorbar;
    ...
    xlabel('Mag. Lat.','FontSize',sized)
    ylabel('Altitude (km)','FontSize',sized);
    ylabel(clb,'N-S Ion Velocity (m/s)','FontSize',sized)
    if strcmp(plotmode,'auto')
        caxis('auto')
    else
        caxis(v2range)
    ...
    h=imagesc(mlon,mlat,parmp2ne(:,:,2));
    set(h,'alphadata',~isnan(parmp2ne(:,:,2)));
    set(gca,'FontSize',sized)
    hold on
    ...
    ylabel(clb,['Electron Density (log_{10} m^{-3}) at ',num2str(altref),' km'],'FontSize',sized)
    if strcmp(plotmode,'auto')
        caxis('auto')
    else
        caxis(nerange)
    ...
    ttxt_prime = {datestr(t,1), [datestr(t,13),' UT (Autocolored)']};
    if strcmp(plotmode,'auto')
        title(ttxt_prime,'FontSize',sized)
    else
        title(ttxt,'FontSize',sized)
    end
    %%%%% ADD ARC BOUNDARY TO NE PLOT? %%%%%
    
    cat1=subplot(3,3,3);
    h=imagesc(mlat,zp,parmp3P);
    set(h,'alphadata',~isnan(parmp3P));
    set(gca,'FontSize',sized)
    axis xy
    clb=colorbar;
    colormap(cat1,'parula')
    ...
    xlabel('Mag. Lat.','FontSize',sized)
    ylabel('Altitude (km)','FontSize',sized);
    ylabel(clb,'|J_P| (log_{10} A/m^2)','FontSize',sized)
    if strcmp(plotmode,'auto')
        caxis('auto')
    else
        caxis(JPrange)
    ...
    cat1=subplot(3,3,6);
    h=imagesc(mlat,zp,parmp3H);
    set(h,'alphadata',~isnan(parmp3H));
    set(gca,'FontSize',sized)
    axis xy
    clb=colorbar;
    colormap(cat1,'parula')
    ...
    xlabel('Mag. Lat.','FontSize',sized)
    ylabel('Altitude (km)','FontSize',sized);
    ylabel(clb,'|J_H| (log_{10} A/m^2)','FontSize',sized)
    if strcmp(plotmode,'auto')
        caxis('auto')
    else
        caxis(JHrange)
    ...
    cat3=subplot(3,3,9);
    h=imagesc(mlat,zp,-parmp3);   % forced the sign convention
    ...
    xlabel('Mag. Lat.','FontSize',sized)
    ylabel('Altitude (km)','FontSize',sized);
    ylabel(clb,'J_{fac} (A/m^2)','FontSize',sized)
    autorange = max(abs(parmp3(:)));
    if strcmp(plotmode,'auto')
        caxis([-autorange,autorange])
    else
        caxis(Jparrange)
    end
...
%rmpath /home/zettergm/Projects/GEMINI-scripts/postprocess;
%rmpath /home/zettergm/Projects/GEMINI/setup;
%rmpath /home/zettergm/Projects/GEMINI/vis;
